...
function(object, object2 = NULL, B = 49, ...) {
    ...
    if(!is.null(object2)){
        ...
        if(ind.rasch.obj2 && is.null(object$constraint))
            stop("\n", deparse(substitute(object)), " must be a constrained Rasch model in order to be nested in ", deparse(substitute(object2)))
        ...
        nb1 <- if(ind.rasch.obj2) nrow(object2$coef) + 1 else nrow(object2$coef) * ncol(object2$coef)
        if(!is.null(constr <- object$constraint)) nb0 <- nb0 - nrow(constr)
        if(!is.null(constr <- object2$constraint)) nb1 <- nb1 - nrow(constr)
        df. <- nb1 - nb0
        LRT <- -2 * (L0 - L1)
        if(LRT < 0 || df. < 0) warning("it seems that the two models are not nested.")
        ...
        if(any(is.na(object$X)) && is.null(object$na.action))
            stop("In case of missing values the parametric Bootstrap test is not implemented.\n")
