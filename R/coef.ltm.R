...
function (object, standardized = FALSE, ...) 
{
    ...
    if (standardized) {
        ...
        if (any(strct$inter, strct$quad.z1, strct$quad.z2)) {
            ...
            dimnames(cof) <- list(paste("Item", 1:nrow(cof)), 
                c("(Intercept)", object$ltn.struct))
            return(round(cof, 2))
        }
        std.z <- cof[, -1]/sqrt(rowSums(cof[, -1, drop = FALSE] * 
            cof[, -1, drop = FALSE]) + factors)
        if (factors == 1) {
            cof <- cbind(cof, std.z)
            dimnames(cof) <- list(paste("Item", 1:nrow(cof)), 
                c("(Intercept)", "z1", "std.z1"))
            return(round(cof, 2))
        }
        else {
