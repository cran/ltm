"ltm.fit" <-
function (X, betas, constraint, factors, inter, quad.z1, quad.z2, control) {
    n <- nrow(X)
    p <- ncol(X)
    ...
    Z <- cbind(1, GH$x)
    GHw <- GH$w * exp(rowSums(dnorm(Z[, seq(2, factors + 1), drop=FALSE], log=TRUE)))
    ...
    if(!is.null(constraint)) res.EM[constraint[1], 1 + constraint[2]] <- NA
    gc()
    res.qN <- optim(c(res.EM[!is.na(res.EM)]), fn=logLik.ltm, gr=score.ltm, method=control$method, hessian=TRUE, 
    ...
    list(coefficients = betas, log.Lik = -res.qN$val, convergence = res.qN$conv, hessian = hes, 
            counts = res.qN$counts, patterns = pats, GH = list(Z = Z, GHw = GHw), max.sc = max.sc)
}
